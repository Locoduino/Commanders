...
unsigned long		   DCC_Decoder::gInterruptMonitorCounter = 0;
...
void DCC_Decoder::DCC_Interrupt()
{
	...
	gInterruptMonitorCounter++;

	if (gInterruptMonitorCounter > 1000)
	{
		if( func_InterruptMonitor )
			(func_InterruptMonitor)();

		gInterruptMonitorCounter = 0;
	}
}
...
//////////////////////////////////////////////////////////////
// Added to blink led on received data

InterruptMonitor DCC_Decoder::func_InterruptMonitor = NULL;

void DCC_Decoder::SetInterruptMonitor(InterruptMonitor func)
{
	func_InterruptMonitor = func;
}
...
const PROG_CHAR *DCC_Decoder::ResultString(byte resultCode)
{
#ifndef DEBUG_MODE
	static const PROG_CHAR *gResult = "OK";
    
    return gResult;
#else
	static const PROG_CHAR *gResults[] = 
    {
        "OK",
		"OK - Unhandled",
		"OK - Boot",
	...
	static const PROG_CHAR *gErrors[] = 
    {
	...
	static const PROG_CHAR *gErrorsBadCode = "ERROR - Bad result code";
...
int DCC_Decoder::GetAddress(int inAddress, int inData)
{
    inAddress -= 1;
    inAddress *= 4;
    inAddress += 1;
    inAddress += (inData & 0x06) >> 1;
