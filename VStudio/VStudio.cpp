...
void setup()
{
	uint16_t val = COMMANDERSCONFIG(25, 43);

	uint8_t cvadd = COMMANDERSCONFIGADDRESS(val);
	uint8_t cvval = COMMANDERSCONFIGVALUE(val);
...
//	SerialCommander.begin(115200);
	...
	switch_light.AddEvent(16, 404, COMMANDERS_EVENT_MOVE, COMMANDERS_MOVE_LEFT);	// light on
	switch_light.AddEvent(17, 405, COMMANDERS_EVENT_MOVE, COMMANDERS_MOVE_STOP);	// light off
	switch_light.begin();
